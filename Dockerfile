...
#
# Generated on 2026-07-15T20:55:06Z by kres d66531d.
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=kube-service-exposer/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=kube-service-exposer/go/pkg GOARCH=amd64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=kube-service-exposer -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /kube-service-exposer-linux-amd64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=kube-service-exposer/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=kube-service-exposer/go/pkg GOARCH=arm64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=kube-service-exposer -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /kube-service-exposer-linux-arm64
